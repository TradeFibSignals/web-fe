...
// Get completed signals from database
...
export async function calculateSignalStats(
  ...
  try {
    let query = supabase.from("completed_signals").select("*")
    ...
    if (error) {
      console.error("Error fetching signals for stats:", error)
      return {
        totalSignals: 0,
        winningSignals: 0,
        losingSignals: 0,
    ...
    const winningSignals = data.filter((signal) => signal.profit_loss > 0).length
    const losingSignals = data.filter((signal) => signal.profit_loss < 0).length

    // Win rate
    const winRate = totalSignals > 0 ? (winningSignals / totalSignals) * 100 : 0

    // Profit/loss calculations
    const winningTrades = data.filter((signal) => signal.profit_loss > 0)
    const losingTrades = data.filter((signal) => signal.profit_loss < 0)

    const totalProfit = winningTrades.reduce((sum, signal) => sum + signal.profit_loss, 0)
    const totalLoss = Math.abs(losingTrades.reduce((sum, signal) => sum + signal.profit_loss, 0))
    const totalProfitLoss = totalProfit - totalLoss

    // Average profit/loss percentages
    const averageProfitPercent =
      winningTrades.length > 0
        ? winningTrades.reduce((sum, signal) => sum + signal.profit_loss_percent, 0) / winningTrades.length
        : 0

    const averageLossPercent =
      losingTrades.length > 0
        ? Math.abs(losingTrades.reduce((sum, signal) => sum + signal.profit_loss_percent, 0)) / losingTrades.length
        : 0
    ...
    // Largest win/loss
    const largestWin = winningTrades.length > 0 ? Math.max(...winningTrades.map((signal) => signal.profit_loss)) : 0

    const largestLoss =
      losingTrades.length > 0 ? Math.abs(Math.min(...losingTrades.map((signal) => signal.profit_loss))) : 0

    // Average RRR
    const averageRRR =
      totalSignals > 0 ? data.reduce((sum, signal) => sum + signal.risk_reward_ratio, 0) / totalSignals : 0
