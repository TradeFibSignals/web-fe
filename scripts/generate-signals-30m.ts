// Skript pro generování signálů pro 30m timeframe
...
async function main() {
  try {
    ...
    // Spuštění generování signálů pro 30m timeframe
    console.log("Starting signal generation for 30m timeframe...")
    ...
    const response = await fetch(`${apiUrl}/api/signals/generate`, {
      ...
      body: JSON.stringify({ timeframe: "30m" }),
    })
    ...
    console.log("Checking signal statuses for 30m timeframe...")

    const statusResponse = await fetch(`${apiUrl}/api/signals/generate?timeframe=30m`, {
      method: "GET",
      headers: {
        "x-api-key": apiKey,
      },
    ...
    console.log("Signal generation process completed successfully for 30m timeframe")
  } catch (error) {
